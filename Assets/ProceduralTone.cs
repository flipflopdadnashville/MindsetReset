...
public class ProceduralTone : MonoBehaviour {
    ...
    void OnAudioFilterRead(float[] data, int channels)
    {
        ...
        for (int i = 0; i< data.Length; i++)
        {
            ...
            // phase = phase + increment;                            // If you count your own phase (kind of like a timer), you dont have to deal with precision issue of float, however if you are playing several frequencies, each would require its own phase, which can get annoying
    ...
    void Update () {
        ...
        if(Input.GetKey(KeyCode.Period)){
            ad_source.volume = ad_source.volume + .1f;  
        } 
        ...
        if(Input.GetKey(KeyCode.LeftBracket)){
            toneFrequency = toneFrequency - 1f;  
        } 
        ...
        // Pan - deoesn't seem to affect anything. Don't care at this point.
        if(Input.GetKey(KeyCode.LeftShift)){
            ad_source.panStereo = ad_source.panStereo - .1f;  
        } 
