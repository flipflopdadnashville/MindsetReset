...
public class DynamicWalls : MonoBehaviour
{
    ...
    void Update(){
        if(this.gameObject.GetComponent<Camera>().enabled == true && wallsCreated == false){
            ...
            Vector3 cameraPos = this.gameObject.GetComponent<Camera>().transform.position;
            screenSize.x = Vector2.Distance (this.gameObject.GetComponent<Camera>().ScreenToWorldPoint(new Vector2(0,0)),this.gameObject.GetComponent<Camera>().ScreenToWorldPoint(new Vector2(Screen.width, 0))) * 0.5f; //Grab the world-space position values of the start and end positions of the screen, then calculate the distance between them and store it as half, since we only need half that value for distance away from the camera to the edge
            screenSize.y = Vector2.Distance (this.gameObject.GetComponent<Camera>().ScreenToWorldPoint(new Vector2(0,0)),this.gameObject.GetComponent<Camera>().ScreenToWorldPoint(new Vector2(0, Screen.height))) * 0.5f;
            ...
            //Change positions to align perfectly with outer-edge of screen, adding the world-space values of the screen we generated earlier, and adding/subtracting them with the current camera position, as well as add/subtracting half out objects size so it's not just half way off-screen
