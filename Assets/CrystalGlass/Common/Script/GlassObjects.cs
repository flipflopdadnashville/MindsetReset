...
public class GlassObjects : MonoBehaviour
{
	...
	public bool m_UseReflectionProbe = false;
	...
	int m_UseShaderIndex = 1;

	void Start ()
	{
		Renderer rd1 = this.gameObject.GetComponent<Renderer> ();
		int n = rd1.materials.Length;
		m_Mats = new Material[n];
		m_Mats[0] = rd1.materials[0];
        m_UseShaderIndex = 3;
        m_Mats[0].EnableKeyword ("CRYSTAL_GLASS_SPARKLE");
        m_Mats[0].EnableKeyword ("CRYSTAL_GLASS_BUMP");
	}
	...
	void Update ()
	{
		for (int i = 0; i < m_Mats.Length; i++)
		{
			...
			if (m_UseReflectionProbe)
				m_Mats[i].EnableKeyword ("CRYSTAL_GLASS_ReflectionProbe");
			else
				m_Mats[i].DisableKeyword ("CRYSTAL_GLASS_ReflectionProbe");
		}
	}
	// void OnGUI()
	// {
