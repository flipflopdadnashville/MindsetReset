...
namespace SimpleFileBrowser
{
	public class FileBrowser : MonoBehaviour, IListViewAdapter
	{
		...
		public GameObject musicPlayerApp;
		public GameObject soundscapeCreatorApp;
		public GameObject mindsetSelectorApp;
...
#pragma warning restore 0414
		...
		[SerializeField]
		public GameObject eqBackWall;

		[SerializeField]
		public GameObject eqFrontWall;
...
#pragma warning restore 0414
		...
		#region Properties
		public string m_currentPath = string.Empty; //Application.streamingAssetsPath.ToString(); //"C:\\Unity\\RestAndRelax\\Assets\\Resources\\AudioSamples";
		...
		private void Awake()
		{
			...
			//If sounds aren't loading, have to check file types
			#if UNITY_EDITOR || UNITY_ANDROID || UNITY_STANDALONE_WIN
						eAudioSampleFiles = Directory.EnumerateFiles(Application.streamingAssetsPath + "\\Sound\\AudioSamples", "*.wav", SearchOption.AllDirectories);
			#elif UNITY_STANDALONE_OSX || UNITY_IOS
						eAudioSampleFiles = Directory.EnumerateFiles(Application.streamingAssetsPath + "/Sound/AudioSamples", "*.wav", SearchOption.AllDirectories);
			#endif

			AudioSampleFiles = eAudioSampleFiles.ToList();
			/*Debug.Log("AudioSampleFiles count is: " + AudioSampleFiles.Count);

			foreach(string file in AudioSampleFiles){
			     Debug.Log(file);
			}*/
		...
		private void OnApplicationFocus( bool focus )
		{
			if( !focus )
				...
				//JWR stuff on line 938. NOTE - this CurrentPath variable is the proper way to set the path, because it adds the path the List which makes the back and forward button work
				//20220817 - looks like I don't even need this next line if I set the variable properly. Go figure.
				//CurrentPath = (Application.streamingAssetsPath + "\\Sound").Replace("/", "\\");

				SetCurrentPathBasedOnApplication();
				RefreshFiles( true );
		}
		#endregion
		...
		{
			// audioSources[0].Stop();
			// Get the channel we're setting
			//string filterDropdownText = filtersDropdown.GetComponent<Text>().text;
			//int dropDownTextInt = Int16.Parse(filterDropdownText);

			// foreach(AudioSource source in audioSources){
			// 	source.Stop();
			// }

			int channelDropdownValue = channelDropdown.value;
			//Debug.Log("Setting audio for channel: " + (channelDropdownValue + 1));
			string filenameInput = filenameInputField.text;
			string[] filenameInputSplit = filenameInput.Split('.');
			string filename = filenameInputSplit[0];
			//Debug.Log("Select button Clicked. Filename selected is: " + filename);
			string folderName = filename.Remove(filename.Length - 2, 2);
			//Debug.Log("Folder name is: " + folderName);
			//string filepath;
			//Debug.Log("Path to load is: " + filepath);
			#if UNITY_EDITOR || UNITY_STANDALONE_WIN
				// filepath = "AudioSamples/" + folderName + "/" + filename;
				// audioSources[channelDropdownValue].clip = Resources.Load<AudioClip>(filepath);
				StartCoroutine(LoadAudio(audioSources[channelDropdownValue], false, false));
			#elif UNITY_STANDALONE_OSX || UNITY_IOS
				// filepath = "file://" + folderName + "/" + filename;
				// LoadAudioForMac(filepath, folderName, filename, channelDropdownValue);
				StartCoroutine(LoadAudio(audioSources[channelDropdownValue], false, false));
			#endif

			//foreach(AudioSource source in audioSources){
				audioSources[channelDropdownValue].Play();
			//}
			//audioSources[channelDropdownValue].Play();
		}
		...
		public void OnModeButtonClicked(){
			...
			foreach(AudioSource source in audioSources){
				if(source.spatialBlend == 0){
					...
					//eqFrontWall.transform.position = new Vector3(eqBackWall.transform.position.x, eqBackWall.transform.position.y, 10);
					//eqBackWall.transform.position = new Vector3(eqBackWall.transform.position.x, eqBackWall.transform.position.y, 120);
					modeButton.GetComponent<Image>().color = Color.green;
				}
				else{
					source.spatialBlend = 0;
					//eqFrontWall.transform.position = new Vector3(eqBackWall.transform.position.x, eqBackWall.transform.position.y, 74);
					//eqBackWall.transform.position = new Vector3(eqBackWall.transform.position.x, eqBackWall.transform.position.y, 80);
		...
		public void OnPresetButtonClicked()
		{
			...
			string pathNameInput = pathInputField.text;
			//Debug.Log("pathName is: " + pathNameInput);
#if UNITY_EDITOR || UNITY_STANDALONE_WIN
				string[] pathNameInputSplit = pathNameInput.Split('\\');
#elif UNITY_STANDALONE_OSX || UNITY_IOS
				string[] pathNameInputSplit = pathNameInput.Split('/');
#elif UNITY_WEBGL
				string[] pathNameInputSplit = pathNameInput.Split('\\');
#endif
			//Debug.Log("string array length is: " + pathNameInputSplit.Length);

			//foreach(string segment in pathNameInputSplit){
			//Debug.Log("segment is: " + segment);
			//}
			//string foldername = pathNameInputSplit[6];
			string foldername = pathNameInputSplit.Last();
			//Debug.Log("Preset button Clicked. Folder selected is: " + foldername);

			//int i = -1;
			//foreach(AudioSource source in audioSources){
			//	i++;
			//	string filepath = "AudioSamples/" + foldername + "/" + foldername + i + "a";
			//	Debug.Log("Path to load is: " + filepath + " - " + source.name);
				// source.clip = Resources.Load<AudioClip>(filepath);
			StartCoroutine(LoadAudio(audioSources[0], true, false));
			//}
			...
			//SetNoiseGeneratorPresetFilename(foldername);
		}

		public void OnRandomButtonClicked(){
			StartCoroutine(LoadAudio(audioSources[0], true, true));
		}

		// public void SetNoiseGeneratorPresetFilename(string name){
		// 	presetFilename = name;
		// }

		// public string GetNoiseGeneratorPresetFilename(){
		// 	return presetFilename;
		// }

		// public void SetMusicPlayerAudioSource(){
		// 	StartCoroutine(LoadAudio(musicPlayerAudioSource, false, false));	
		// }
		...
		// Problem here on Mac right now. Probably bad url. Uncommented Debug, need to rebuild
		IEnumerator LoadAudio(AudioSource audioSource, bool allSources, bool random) 
		{ 
			int k = 0;
			string url;
			WWW www;
			List<string> currentlyPlayingList = new List<string>();

			if(allSources == false){
				//string url;
				//WWW www;
				string pathNameInput = pathInputField.text.Trim();
				string musicFilePath = filenameInputField.text;
				try{
					url = "file://" + pathNameInput + "/" + musicFilePath;
					//Debug.Log("url is: " + url);
					www = new WWW(url);				
				}
				catch{
					url = string.Format("file://" + Application.streamingAssetsPath + "/Sound/{0}", musicFilePath);
					//Debug.Log("That failed. url is: " + url);
					www = new WWW(url);
				}
				
				//Debug.Log("Trying to load and play clip: " + url);
				yield return www;

				if(musicPlayerAudioSource.enabled == false){
					musicPlayerAudioSource.enabled = true;
				}

				audioSource.clip = www.GetAudioClip(false, false);
				audioSource.Play();
				currentlyPlayingList.Add(url.Split(@"\").Last());
			}
			else if(allSources == true){
				UnloadAudio();
				if(random == false){
				//Debug.Log("Going to try to set all the audio sources... There are " + audioSources.Length + " to set.");

					foreach(AudioSource source in audioSources){
						if(k > audioSources.Length){
							k = 0;
						}
						...
						//Debug.Log("string array length is: " + pathNameInputSplit.Length);
						//string foldername = pathNameInputSplit[7];
						string foldername = pathNameInputSplit.Last();
						//Debug.Log("Last segment of path is: " + foldername);

						
						string musicFilePath = "AudioSamples/" + pathNameInput + "/" + foldername + k + "a.mp3";
						try{
							url = string.Format("file://" + pathNameInput + "/" + foldername + k + "a.mp3");
							//Debug.Log("url is: " + url);
							www = new WWW(url);
						}
						catch{
						...
						k++;
						currentlyPlayingList.Add(foldername + k + "a");
						//Debug.Log(k);
					}
				}
				else if(random == true){
			...
			SetCurrentlyPlayingText(currentlyPlayingList);
		}

		private void SetCurrentlyPlayingText(List<string> currentlyPlayingList){
			CurrentlyPlayingText.text = "";

			foreach(string item in currentlyPlayingList){
				CurrentlyPlayingText.text += item.Split(@".")[0];
			}
		}

		//JWR add-in. Wonky, but I'm tired. I'll refactor it later (he said half-heartedly)
		public void SetCurrentPathBasedOnApplication(){
#if UNITY_EDITOR || UNITY_STANDALONE_WIN
			// need to clear the path followed in case both are open. Don't want cross-contamination
			if(musicPlayerApp.activeInHierarchy == true){
				//Debug.Log("Music Player active");
				CurrentPath = (Application.streamingAssetsPath + "\\Sound\\Songs").Replace("/", "\\");
			}
			
			if(soundscapeCreatorApp.activeInHierarchy == true){
				//Debug.Log("Soundscape Creator active");
				CurrentPath = (Application.streamingAssetsPath + "\\Sound\\AudioSamples").Replace("/", "\\");
			}

			if(mindsetSelectorApp.activeInHierarchy == true){
				//Debug.Log("Soundscape Creator active");
				CurrentPath = (Application.streamingAssetsPath + "\\Data\\Alternate").Replace("/", "\\");
			}
...
// 		}
		...
		public void Show( string initialPath, string initialFilename )
		{
			...
			Debug.Log("From Show (1927) current path is: " + CurrentPath);
		...
		public void RefreshFiles( bool pathChanged )
		{
			...
			// JWR setting filepath - I may have broken things so this is my fix
			pathInputField.text = m_currentPath;
		}
